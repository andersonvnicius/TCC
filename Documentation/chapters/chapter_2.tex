...
%			Programação & Software próprio & Software próprio & Software próprio \\
			Taxa de transferência de dados & Não disponiblizado & Não disponiblizado & $\SI{4}hz$ \\
...
Os resultados completos dos artigos encontrados e avaliados pela revisão bibliográfica podem ser encontrados no \autoref{ch:RB}
...
\begin{table}[!ht]
    ...
    \resizebox{\textwidth}{!}{%
		...
		\multicolumn{1}{l}{\textbf{Módulos de conversão de sinal}} & \\
			HX711 & Desenvolvido para aplicações de célula de carga & $R\${20.00}$ \\
			ADS1115 & Alta taxa de obtenção de dados & $R\${70.00}$ \\
...
\begin{figure}[H]
	\caption{\label{fig:2046} Segundo protótipo preparado para a realização dos experimentos}
		\begin{center}
			\includegraphics[width=\textwidth]{pictures/2046.png}
		\end{center}
	\fonte{O autor 2022}
\end{figure}
...
\begin{table}[!ht]
    ...
    \resizebox{350}{!}{%
        \begin{tabular}{ l | c c r } \toprule
            ...
            Peso a & $\SI{497.66}{\g}$ &$\SI{497.69}{\g}$ & $\SI{0.02}{\%}$ \\
...
\begin{lstlisting}[label={lst:handle_read},language=C++,caption={{Função de obtenção de sinal pelo módulo de conversão HX711}}]
...
\begin{figure}[htb]
	...
	\begin{center}
		\includegraphics[width=600, angle=90]{pictures/2043.png}
	\end{center}
	\fonte{O autor 2022}
\end{figure}
...
\begin{lstlisting}[label={lst:objectcalibration},language=Python,caption={{Métodos de calibração do objeto NetworkDevice}}]
