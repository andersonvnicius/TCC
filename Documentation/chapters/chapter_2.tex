...
O projeto elétrico do dispositivo foi feito no programa ... e é mostrado na \autoref{fig:2041}

\begin{figure}[htb]
	\caption{\label{fig:2041} Projeto elétrico do dispositivo}
	\begin{center}
		\includegraphics[width=\textwidth]{pictures/11 - ESP32 Device wiring.jpg}
	\end{center}
	\fonte{O autor 2022}
\end{figure}
...
O programa completo do controlador completo é apresentado no \autoref{appendix_a}. % TODO como referenciar APENDICES
...
\begin{figure}[htb]
	\caption{\label{fig:2043} Objeto SerialDevice}
	\begin{center}
...
\begin{lstlisting}[label={lst:essentialobjectfuncions}][language=Python,caption={[NetworkDevice]{Funções base do objeto NetworkDevice}}]
...
\begin{lstlisting}[label={lst:sample_reading}][language=Python,caption={[Network Device calibrate offset]{'Calibração do parametro de offset do objeto NetworkDevice}}]
...
\begin{lstlisting}[label={lst:objectcalibration}][language=Python,caption={[Network Device calibrate offset]{'Calibração do parametro de offset do objeto NetworkDevice}}]
...
Os códigos completos para o objeto NetworkDevice e para a geração da interface gráfica são apresentados no \autoref{appendix_b}.
